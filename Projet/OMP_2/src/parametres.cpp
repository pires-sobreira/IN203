...
mise_a_jour(const parametres& params, int width, int height, const char* galaxie_previous, char* galaxie_next)
{
    ...
    int amount = height / (omp_get_num_threads()-1);
    int rest = height % (omp_get_num_threads()-1);
    int thread_n = omp_get_thread_num();
    int begin, final;

    if(thread_n-1 < rest) begin = (thread_n-1)*amount +(thread_n-1) ;
    else begin = (thread_n-1)*amount + rest;
    
    if(thread_n < rest) final = thread_n*amount + thread_n ;
    else final = thread_n*amount + rest;


    std::random_device rand_t_device;
    std::default_random_engine rand_t_engine(rand_t_device());
    ...
    for (i = begin ; i < final; ++i)
    {
        for (j = 0; j < width; ++j)
        {
            if (galaxie_previous[i * width + j] == habitee)
            {
                ...
                {
                    expansion e = calcul_expansion(params, (double)(rand_t_engine()));
                ...
                if (calcul_depeuplement(params, (double)(rand_t_engine())))
                {
                    galaxie_next[i * width + j] = habitable;
                }

                if (calcul_inhabitable(params, (double)(rand_t_engine())))
                {
            ...
            {
                if (apparition_technologie(params, (double)(rand_t_engine())))
