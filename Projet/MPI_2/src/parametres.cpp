...
expansion calcul_expansion(const parametres& c, double v)
{
    //double val = std::rand()/(1.*RAND_MAX);
    double val = v/(1.*RAND_MAX);
...
bool calcul_depeuplement(const parametres& c, double v)
{
    //double val = std::rand()/(1.*RAND_MAX);
    double val = v/(1.*RAND_MAX);
...
//_ ______________________________________________________________________________________________ _
bool calcul_inhabitable(const parametres& c, double v)
{
    //double val = std::rand()/(1.*RAND_MAX);
    double val = v/(1.*RAND_MAX);
...
//_ ______________________________________________________________________________________________ _
bool apparition_technologie(const parametres& p, double v)
{
    //double val = std::rand()/(1.*RAND_MAX);
    double val = v/(1.*RAND_MAX);
...
{
    std::random_device rand_t;
    std::default_random_engine rand_for_t(rand_t());
    ...
    for ( i = 1; i < height-1; ++i )
      {
        for ( j = 0; j < width; ++j )
        {
            if (galaxie_previous[i*width+j] == habitee)
            {
                ...
                {
                    expansion e = calcul_expansion(params, (double)(rand_for_t()));
                ...
                if (calcul_depeuplement(params, (double)(rand_for_t())))
                {
                    galaxie_next[i*width+j] = habitable;
                }
                
                if (calcul_inhabitable(params, (double)(rand_for_t())))
                {
            ...
            {
                if (apparition_technologie(params, (double)(rand_for_t())))
...
void MjDebut(int width, int height,char* ghostbegin, char* gnext, char* gold)
{
...
void MjFin(int width, int height,char* ghostend, char* gnext, char* gold)
{
    for (int j = 0; j < 2*width; j++)
    {
