    ...
    for ( i = 0; i <  height; ++i )
      {
        for ( j = 0; j < width; ++j )
        {
            if (galaxie_previous[i*width+j] == habitee)
            {
                ...
                {
                    expansion e = calcul_expansion(params,(double)(rand_for_t()/(1.*RAND_MAX)));
                ...
                if (calcul_depeuplement(params,(double)(rand_for_t()/(1.*RAND_MAX))))
                {
                    galaxie_next[i*width+j] = habitable;
                }
                if (calcul_inhabitable(params,(double)(rand_for_t()/(1.*RAND_MAX))))
                {
            ...
            {
                if (apparition_technologie(params,(double)(rand_for_t()/(1.*RAND_MAX))))
